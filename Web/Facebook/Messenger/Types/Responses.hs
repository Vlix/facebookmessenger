...
instance FromJSON GetProfileResponse where
  parseJSON = withObject "GetProfileResponse" $ \o -> do
      arr <- o .: "data" :: Parser [Object]
      let obj = foldr HM.intersection HM.empty arr
      GetProfileResponse <$> parseJSON (Object obj)
...
instance ToJSON GetProfileResponse where
  toJSON (GetProfileResponse pr) = object [ "data" .= go ]
    where go = toJSON $ catMaybes allFields
          ...
          whitelist = mkObj "whitelisted_domains" <$> prWhitelistedDomains pr
          accountLink = mkObj "account_linking_url" <$> prAccountLinkingUrl pr
          paySettings = mkObj "payment_settings" <$> prPaymentSettings pr
          audience = mkObj "target_audience" <$> prTargetAudience pr
          homeUrl = mkObj "home_url" <$> prHomeUrl pr
          mkObj key val = object [key .= val]
