  ...
  toJSON abp = object' $ extra : basis
   where
    extra = case abpQrBarCode abp of
      AirlineQR (AirlineQRCode code) -> "qr_code" .=! code
      AirlineBar (AirlineBarCode url) -> "barcode_image_url" .=! url
...
instance ToJSON AirlineQRBarCode where
  toJSON (AirlineQR x) = toJSON x
  toJSON (AirlineBar x) = toJSON x
...
instance FromJSON AirlineQRBarCode where
  parseJSON = withObject "AirlineQRBarCode" $ \o ->
        AirlineQR <$> parseJSON (Object o)
    <|> AirlineBar <$> parseJSON (Object o)
