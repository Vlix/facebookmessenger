...
newtype ShareContents = ShareContents { scElement :: GenericElement }
  deriving stock (Eq, Show, Read, Ord)

instance FromJSON ShareContents where
  parseJSON = withObject "ShareContents" $ \o -> do
      att <- o .: "attachment"
      checkValue "ShareContents:TemplatePayload"
                 "type"
                 ("template" :: Text)
                 go
                 att
    where go obj = do
              pl <- obj .: "payload"
              checkValue "ShareContents:GenericTemplate"
                         "template_type"
                         ("generic" :: Text)
                         go2
                         pl
          go2 genTempl = do
              es <- genTempl .: "elements"
              let (e:|_) = es
              pure $ ShareContents e

instance ToJSON ShareContents where
  toJSON (ShareContents e) = obj
...
instance FromJSON GenericElement where
  parseJSON = withObject "GenericElement" $ \o -> do
          btns <- o .:? "buttons" .!= []
          (mBuy, restBtns) <- getBtns $ take 3 btns
          GenericElement <$> o .: "title"
                         ...
                         <*> pure mBuy
                         <*> pure restBtns
    where getBtns [] = return (Nothing, [])
          getBtns (TBuy bb:rest) = do
              checkBtns rest
              return (Just bb, rest)
          getBtns btns = do
              checkBtns btns
              return (Nothing, btns)
          checkBtn TBuy{} = fail "can't have buy button as later buttons"
          checkBtn _ = return ()
          checkBtns = mapM_ checkBtn
...
instance ToJSON GenericElement where
  toJSON (GenericElement title subtitle image da buy buttons) =
    ...
    where btnList Nothing = dropBuys buttons
          btnList (Just btn) = TBuy btn : dropBuys buttons
          dropBuy TBuy{} = Nothing
          dropBuy x = Just x
          dropBuys = catMaybes . fmap dropBuy
